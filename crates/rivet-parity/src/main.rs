...
#[command(
  name = "rivet-parity",
  about = "Runs parity scenarios \
           against Rivet task CLI and \
           Taskwarrior"
)]
struct Args {
  #[arg(
    long,
    default_value = "target/debug/task"
  )]
  ...
  #[arg(
    long,
    default_value = "crates/rivet-parity/scenarios/basic_flow.json"
  )]
...
fn main() -> anyhow::Result<()> {
  ...
  for scenario in scenarios {
    ...
    let candidate = run_engine(
      &args.candidate_bin,
      &scenario
    )
    .with_context(|| {
      format!(
        "candidate failed on scenario \
         {}",
        scenario.name
      )
    })?;
    ...
    if reference_available {
      ...
      let pending_score = score_bucket(
        &candidate.pending,
        &reference.pending
      );
      let completed_score =
      ...
      let deleted_score = score_bucket(
        &candidate.deleted,
        &reference.deleted
      );
      let scenario_score =
      ...
      println!(
        "    pending:   {}",
        candidate.pending.len()
      );
      println!(
        "    completed: {}",
        candidate.completed.len()
      );
      println!(
        "    deleted:   {}",
        candidate.deleted.len()
      );
    }
  }

  if !overall_scores.is_empty() {
    let mean = overall_scores
      .iter()
      .sum::<f64>()
      / overall_scores.len() as f64;
    println!(
      "\nOverall parity score: {:.3}",
      mean
    );
  }
...
fn is_reference_available(
  reference_bin: &Path
) -> bool {
  if reference_bin.is_absolute()
    || reference_bin
      .components()
      .count()
      > 1
...
fn run_engine(
  ...
  let temp_dir = TempDir::new()
    .context(
      "failed to create temp scenario \
       dir"
    )?;
  let (taskrc, data_dir) =
    write_taskrc(temp_dir.path())?;
  install_hooks(
    &data_dir,
    &scenario.hooks
  )?;
  ...
  let mut pending = collect_export(
    binary,
    &taskrc,
    &["status:pending"]
  )?;
  pending.extend(collect_export(
    binary,
    &taskrc,
    &["status:waiting"]
  )?);
  pending.sort();
  pending.dedup();
  let completed = collect_export(
    binary,
    &taskrc,
    &["status:completed"]
  )?;
  let deleted = collect_export(
    binary,
    &taskrc,
    &["status:deleted"]
  )?;
...
) -> anyhow::Result<(PathBuf, PathBuf)>
{
  ...
  let content = format!(
    "data.location={}\\
     ...
     rivet=+rivet\ncontext.\
     ops=project:ops\nreport.focus.\
     columns=id,description,urgency\\
     nreport.focus.labels=ID,\
     Description,Urgency\nreport.\
     focus.sort=urgency-\nreport.\
     focus.filter=status:pending\\
     nreport.focus.limit=50\n",
    data_dir.display()
  );
  fs::write(&taskrc, content)
    .with_context(|| {
      format!(
        "failed to write taskrc {}",
        taskrc.display()
      )
    })?;
...
fn canonicalize(
  value: Value
) -> anyhow::Result<CanonicalTask> {
  let obj = value
    .as_object()
    .ok_or_else(|| {
      anyhow!(
        "expected task object in \
         export"
      )
    })?;
...
fn score_bucket(
  candidate: &[CanonicalTask],
  reference: &[CanonicalTask]
) -> f64 {
  let c: BTreeSet<_> =
  ...
  let intersection =
    c.intersection(&r).count() as f64;
  let union =
    c.union(&r).count() as f64;
...
fn print_diff(
  ...
  let only_candidate: Vec<_> =
    c.difference(&r).cloned().collect();
  let only_reference: Vec<_> =
    r.difference(&c).cloned().collect();

  if only_candidate.is_empty()
    && only_reference.is_empty()
  {
    return;
  }
