...
impl Filter {
    ...
    #[tracing::instrument(skip(self, task, now))]
    pub fn matches_without_waiting_guard(&self, task: &Task, now: DateTime<Utc>) -> bool {
...
impl Parser {
    fn new(tokens: Vec<String>, now: DateTime<Utc>) -> Self {
        Self {
            tokens,
            pos: 0,
            now,
    ...
    fn parse_primary(&mut self) -> anyhow::Result<Expr> {
        ...
        let pred = parse_atom(&token, self.now)?;
        Ok(Expr::Pred(pred))
    }
    ...
    fn expect_token(&mut self, expected: &str) -> anyhow::Result<()> {
        if self.match_token(expected) {
            ...
            Err(anyhow::anyhow!(
                "expected '{expected}' in filter expression"
            ))
        }
    }
...
fn parse_atom(term: &str, now: DateTime<Utc>) -> anyhow::Result<Pred> {
    ...
    if let Some(project) = term.strip_prefix("project:") {
        return Ok(Pred::ProjectEq(project.to_string()));
    }

    if let Some(status_text) = term.strip_prefix("status:") {
        return Ok(match status_text.to_ascii_lowercase().as_str() {
            "pending" => Pred::StatusEq(Status::Pending),
            "completed" => Pred::StatusEq(Status::Completed),
            "deleted" => Pred::StatusEq(Status::Deleted),
            "waiting" => Pred::Waiting,
            _ => Pred::TextContains(term.to_string()),
        });
    }

    if let Some(value) = term.strip_prefix("due.before:") {
        return Ok(Pred::DueBefore(parse_date_expr(value, now)?));
    }

    if let Some(value) = term.strip_prefix("due.after:") {
        return Ok(Pred::DueAfter(parse_date_expr(value, now)?));
    }
...
fn eval_pred(pred: &Pred, task: &Task, now: DateTime<Utc>) -> bool {
    let ok = match pred {
        ...
        Pred::VirtualTagInclude(virtual_tag) => eval_virtual_tag(*virtual_tag, task, now),
        Pred::VirtualTagExclude(virtual_tag) => !eval_virtual_tag(*virtual_tag, task, now),
...
fn eval_virtual_tag(virtual_tag: VirtualTag, task: &Task, now: DateTime<Utc>) -> bool {
    ...
    match virtual_tag {
        ...
        VirtualTag::Waiting => task.is_waiting(now),
        VirtualTag::Completed => task.status == Status::Completed,
        VirtualTag::Deleted => task.status == Status::Deleted,
        VirtualTag::Active => {
            task.status == Status::Pending && !task.is_waiting(now) && task.start.is_some()
        }
        VirtualTag::Ready => {
            task.status == Status::Pending && !task.is_waiting(now) && task.depends.is_empty()
        }
        VirtualTag::Blocked => !task.depends.is_empty(),
...
fn parse_virtual_tag(tag: &str) -> Option<VirtualTag> {
    match tag {
        ...
        "WAITING" => Some(VirtualTag::Waiting),
        "COMPLETED" => Some(VirtualTag::Completed),
        "DELETED" => Some(VirtualTag::Deleted),
        "ACTIVE" => Some(VirtualTag::Active),
        "READY" => Some(VirtualTag::Ready),
        "BLOCKED" => Some(VirtualTag::Blocked),
        "UNBLOCKED" => Some(VirtualTag::Unblocked),
        "DUE" => Some(VirtualTag::Due),
        "OVERDUE" => Some(VirtualTag::Overdue),
        "TODAY" => Some(VirtualTag::Today),
        "TOMORROW" => Some(VirtualTag::Tomorrow),
        _ => None,
    }
}

fn expr_has_explicit_status_filter(expr: &Expr) -> bool {
    match expr {
        Expr::True => false,
        ...
        Expr::And(nodes) | Expr::Or(nodes) => nodes.iter().any(expr_has_explicit_status_filter),
    }
}

fn expr_has_identity_selector(expr: &Expr) -> bool {
    match expr {
        Expr::True => false,
        Expr::Pred(pred) => matches!(pred, Pred::Id(_) | Pred::Uuid(_)),
        Expr::And(nodes) | Expr::Or(nodes) => nodes.iter().any(expr_has_identity_selector),
    }
}
...
mod tests {
    ...
    fn boolean_precedence_and_parentheses() {
        ...
        let mut x = Task::new_pending("x".to_string(), now, 1);
        x.tags = vec!["x".to_string()];

        let mut y = Task::new_pending("y".to_string(), now, 2);
        y.tags = vec!["y".to_string()];

        let mut xy = Task::new_pending("xy".to_string(), now, 3);
        xy.tags = vec!["x".to_string(), "y".to_string()];
    ...
    fn virtual_tags_pending_waiting_and_active() {
        let now = Utc.with_ymd_and_hms(2026, 2, 16, 5, 0, 0).unwrap();
        let mut active = Task::new_pending("active".to_string(), now, 1);
        active.start = Some(now);

        let mut waiting = Task::new_pending("waiting".to_string(), now, 2);
        waiting.wait = Some(now + Duration::hours(2));

        let pending_filter = Filter::parse(&["+PENDING".to_string()], now).unwrap();
        let waiting_filter = Filter::parse(&["+WAITING".to_string()], now).unwrap();
        let active_filter = Filter::parse(&["+ACTIVE".to_string()], now).unwrap();

        assert!(pending_filter.matches(&active, now));
        assert!(!pending_filter.matches(&waiting, now));

        assert!(!waiting_filter.matches(&active, now));
        assert!(waiting_filter.matches(&waiting, now));

        assert!(active_filter.matches(&active, now));
        assert!(!active_filter.matches(&waiting, now));
    }
    ...
    fn id_selector_matches_waiting_task() {
        ...
        let filter = Filter::parse(&["2".to_string()], now).unwrap();
        assert!(filter.has_identity_selector());
        assert!(filter.matches(&waiting, now));
    }

    #[test]
    fn raw_matching_can_include_waiting() {
        let now = Utc.with_ymd_and_hms(2026, 2, 16, 5, 0, 0).unwrap();
        let mut waiting = Task::new_pending("waiting".to_string(), now, 1);
        waiting.wait = Some(now + Duration::hours(2));

        let filter = Filter::default();
        assert!(!filter.matches(&waiting, now));
        assert!(filter.matches_without_waiting_guard(&waiting, now));
    }
}
